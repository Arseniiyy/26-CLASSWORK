...
class CarManager:
    ...
    def add_car(self,db:Session,car:CarCreate)-> ArrivedCar:
        ...
        def get_avaible_car(self,db:Session, skip: int = 0,limit: int = 100)-> List[ArrivedCar]:
            return db.query(ArrivedCar).filter(ArrivedCar.is_avaible == True).offset(skip).limit(limit).all()
